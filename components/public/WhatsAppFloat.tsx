...
export default function WhatsAppFloat({ message }: WhatsAppFloatProps) {
  ...
  return (
    <a
      ...
      className="fixed bottom-6 right-6 z-50 flex items-center justify-center bg-[#25d366] text-white rounded-full shadow-lg transition-all duration-300 ease-in-out p-3 h-12"
      style={{
        width: isHovered ? '180px' : '48px',
      }}
      aria-label="Chat with AutoCapital Wheels on WhatsApp"
      title="WhatsApp AutoCapital Wheels"
    >
      <div className="flex items-center gap-2 overflow-hidden whitespace-nowrap">
        ...
        {isHovered && <span className="text-xs font-bold uppercase tracking-wider pl-1">Chat on WhatsApp</span>}
      </div>
    </a>
  );
}
